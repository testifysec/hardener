...
- Residual AVC denials: 1
  - `mosquitto_t` mosquitto_t:capability [setgid] (path= name=)
...
- `cap-setgid` (imports initgroups, setgid) — **NOT observed at runtime: exercise likely does not cover this**
- `cap-setuid` (imports setuid) — **NOT observed at runtime: exercise likely does not cover this**
...
## Supply-chain conformance (party: second)

Observed behavior matches the declaration/baseline exactly.
